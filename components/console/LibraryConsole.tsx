...
export function LibraryConsole() {
  ...
  if (state.kind === "error") return <AuthFrame><h1>{isSignInRequired(state.error) ? "Sign in to Library" : "Couldn’t open Library"}</h1>{isSignInRequired(state.error) ? <><p className={styles.muted}>Use your workspace email to continue.</p><Link className={styles.primary} href={`/sign-in/?returnTo=${encodeURIComponent(requestedWorkspace ? libraryUrl(requestedWorkspace, itemId ?? undefined, revisionId ?? undefined) : "/account/")}`}>Continue with email</Link></> : <><ErrorNotice message={errorMessage(state.error)} /><div className={styles.inlineNote}><button className={styles.secondary} onClick={retry}>Try again</button></div></>}</AuthFrame>;
  const workspace = state.session.workspaces.find((entry) => entry.id === requestedWorkspace);
  if (!workspace) return <ConsoleFrame session={state.session}><h1>Workspace unavailable</h1><p className={styles.muted}>This workspace isn’t available to your account.</p><Link className={styles.primary} href="/account/">Choose a workspace</Link></ConsoleFrame>;
