...
export type ActiveDestination = "apps" | "library" | "team";
...
function NavIcon({ kind }: { kind: ActiveDestination | "workspaces" }) {
  return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    {kind === "apps" ? <><rect x="3" y="3" width="7" height="7" rx="1.5" /><rect x="14" y="3" width="7" height="7" rx="1.5" /><rect x="3" y="14" width="7" height="7" rx="1.5" /><rect x="14" y="14" width="7" height="7" rx="1.5" /></> : kind === "library" ? <><path d="M4 4h6a3 3 0 0 1 3 3v14a4 4 0 0 0-4-2H4z" /><path d="M13 7a3 3 0 0 1 3-3h4v15h-3a4 4 0 0 0-4 2" /></> : kind === "team" ? <><circle cx="9" cy="8" r="3" /><path d="M3 21v-3a6 6 0 0 1 12 0v3M17 5a3 3 0 0 1 0 6m2 10v-3a6 6 0 0 0-2-4" /></> : <><rect x="4" y="4" width="16" height="16" rx="3" /><path d="M4 10h16M10 10v10" /></>}
  </svg>;
}
...
function WorkspaceLinks({ session, workspace, active, onNavigate }: { session: Session; workspace?: Workspace; active: ActiveDestination; onNavigate?: () => void }) {
  ...
  ] satisfies { id: ActiveDestination; label: string; href: string }[];
    ...
    <div className={styles.workspaceSwitcher}>
      <label>Workspace
        <select value={workspace?.id ?? ""} onChange={(event) => {
          ...
          router.push(!id ? "/workspaces/" : active === "apps" ? workspaceUrl(id) : `/workspace/${active}/?workspace=${encodeURIComponent(id)}`);
          onNavigate?.();
        }}>
...
export function ConsoleFrame({ session, workspace, children, active = "apps" }: { active?: ActiveDestination; session: Session; workspace?: Workspace; children: ReactNode }) {
  ...
  return <div className={`${styles.console} ${styles.workspacePage}`}>
    ...
    <div className={styles.workspaceBody}>
      <div className={styles.desktopBar}><span>{workspace?.name ?? "Your workspaces"}<span className={styles.barDivider}>/</span>{workspace ? active === "library" ? "Company knowledge" : active === "team" ? "People & access" : "Apps" : "Overview"}</span><div><Link href="/auth/device/">Connect an agent</Link><Link href="/docs/">Help ↗</Link></div></div>
...
export function WorkspaceShell({ children }: { children: ReactNode }) {
  ...
  const active = path.startsWith("/workspace/library") ? "library" : path.startsWith("/workspace/team") ? "team" : "apps";
