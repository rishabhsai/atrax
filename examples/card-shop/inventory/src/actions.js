...
export const actions = {
  'stock.list': {
    description: 'Read current stock: each item with its grade or condition, list price in cents, and quantity available to reserve.',
    effect: 'read', inputSchema: object({}),
    outputSchema: object({ items: { type: 'array', items: object({ sku: text, name: text, grade: text, priceCents: { type: 'integer', minimum: 0 }, available: { type: 'integer', minimum: 0 } }) } }),
    async handler(_input, { db }) { return { items: (await db.prepare('SELECT sku,name,grade,price_cents AS priceCents,available FROM stock ORDER BY price_cents DESC,sku').all()).results }; },
  },
  'stock.reserve': {
