...
function parse(args) {
  ...
  for (let i=0;i<args.length;i++) {
    ...
    if (!['template','assets','actions','migrations','port','agent','slug','input','key','workspace','title','type','revision','reason','out','access'].includes(flag)) throw new Error(`Unknown option: --${flag}`);
  ...
  try {
    ...
    switch(command) {
      ...
      case 'workspace': {
        ...
        else if(p[0] === 'create') result=await operation('workspaces.create',{name:p[1],slug:options.slug});
        else if(p[0] === 'use') {
