  ...
  --input '{"appId":"<app-id>"}' --json`}</code></pre>
      ...
      <section className="section shell rule-top">
        <div className="section-split">
          ...
          <div className="cli-details"><h3>Beyond the shortcuts</h3><p><code>atrax call &lt;operation&gt;</code> covers team invitations, app and action audiences, guest access, sessions, backups, and recovery. Use <a href="/operations.json">the operation schemas</a> for exact input and output shapes.</p><h3>Current limits</h3><p>Some CLI shortcuts still need consistent retry-key support. For automated writes such as workspace creation, use the corresponding <code>atrax call</code> operation with <code>--key</code>. Hosted agents and scheduled automation are planned for later.</p></div>
        </div>
      </section>
